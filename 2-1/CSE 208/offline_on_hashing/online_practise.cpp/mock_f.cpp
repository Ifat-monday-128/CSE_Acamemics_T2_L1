...
ll hash1(const string& word) {
    ...
    for (int i = 0; i < (int)word.size(); i++)
        hashValue += (i + 1) * (unsigned char)word[i];
    return hashValue % 13;
}
...
private:
    ...
    void table_shrink() {
        ...
        cout << "*** SHRANK " << oldCapacity << " -> " << capacity() << " ***\n";
    }
    void shrink_check() {
    ...
    void table_grow() {
        ...
        insertsSinceGrow = 0;
        cout << "*** GREW " << oldCapacity << " -> " << capacity() << " ***\n";
    }
...
int main() {
    ...
    vector<string> words = {"kite", "lamp", "frog", "zeal", "gnome", "wolf", "idea", "plum", "dust"};

    for (int i = 0; i < (int)words.size(); i++) {
        int oldCapacity = table.capacity();
        table.insert(words[i], i + 1);
        cout << "insert " << words[i] << " -> "
             << "count=" << table.getCountItem()
             << " capacity=" << table.capacity()
             << " load=" << fixed << setprecision(4) << table.loadFactor();
        if (table.capacity() != oldCapacity)
            cout << " *** GREW " << oldCapacity << " -> " << table.capacity() << " ***";
        cout << "\n";
    }

    vector<string> toDelete = {"kite", "lamp", "frog", "zeal", "gnome", "wolf"};
    for (int i = 0; i < (int)toDelete.size(); i++) {
        int oldCapacity = table.capacity();
        table.erase(toDelete[i]);
        cout << "delete " << toDelete[i] << " -> "
             << "count=" << table.getCountItem()
             << " capacity=" << table.capacity()
             << " load=" << fixed << setprecision(4) << table.loadFactor();
        if (table.capacity() != oldCapacity)
            cout << " *** SHRANK " << oldCapacity << " -> " << table.capacity() << " ***";
        cout << "\n";
    }
}
