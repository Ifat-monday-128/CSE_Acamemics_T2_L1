...
public:
    ...
    int homeSlot(const Key& key) const {
        if (method == CHAINING) return -1;
        return (int)(hash_function(key) % capacity());
    }
    ...
    int clusterLength(const Key& key) const {
        ...
        int N = capacity();
        int home = homeSlot(key);
        int length = 0;

        for (int offset = 0; offset < N; offset++) {
            int idx = (home + offset) % N;
            if (table_probe[idx].state == EMPTY) break;
            if (table_probe[idx].state == OCCUPIED || table_probe[idx].state == DELETED) {
                length++;
            }
        }
        return length;
    ...
    void printClusterLength(const Key& key) {
        if (method == CHAINING) {
            cout << "printClusterLength is only supported for open-addressing tables.\n";
            return;
        }

        int home = homeSlot(key);
        int length = clusterLength(key);
        Value foundValue = Value();
        int hits = 0;
        search(key, foundValue, hits);
...
int main() {
    ...
    HashTable<string, int> table(DOUBLE_HASHING, hash1);
    for (int i = 0; i < (int)reportWords.size(); i++) {
        table.insert(reportWords[i], i + 1);
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        int home = table.homeSlot(key);

        int value = 0;
        int hits = 0;
        if (table.search(key, value, hits)) {
        ...
        cout << key << " -> home=" << home << ", clusterLength=" << length << '\n';
    }

    if (bestKey.empty()) {
        cout << "Maximum cluster length: 0 (key: none)\n";
    } else {
        cout << "Maximum cluster length: " << bestLength
